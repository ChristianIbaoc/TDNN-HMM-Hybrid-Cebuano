...
if $speed_perturb; then
  ...
  if [ $stage -le 2 ] && $generate_alignments; then
    ...
    steps/align_fmllr.sh --nj 100 --cmd "$train_cmd" \
      data/${train_set}_sp data/lang exp/tri3 exp/tri4_ali_nodup_sp
      #CHANGE exp/tri3 to exp/tri4 if DOUBLE DELTAS
  fi
fi
...
if [ $stage -le 3 ]; then
  ...
  for dataset in $train_set trainaug; do
...
if [ $stage -le 5 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    ...
    5500 90000 data/trainaug_hires \
...
if [ $stage -le 6 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 200000 \
    data/${train_set} 512 exp/nnet3/tri3b exp/nnet3/diag_ubm
fi
...
if [ $stage -le 7 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/trainaug_hires exp/nnet3/diag_ubm exp/nnet3/extractor || exit 1;
fi
